#!/bin/bash
# Script to mint a token in Cardano Preprod Testnet
# Usage: ./mint-token.sh <token name> <token amount>

testnet="--testnet-magic 1"

# Check if token name and amount are provided
if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: ./mint-token.sh <token name> <token amount>"
    exit 1
fi
...
sync_progress=$(cardano-cli query tip $testnet | grep "syncProgress" | cut -d ':' -f 2 | tr -d ' "')
...
total=0
balance=$(cardano-cli query utxo --address $address $testnet | grep "lovelace" | awk '{print $3}')
...
utxo=$(cardano-cli query utxo --address $address $testnet | awk '$4 == "lovelace" && NF == 6 && $3 >= 2000000 {print $1, $2, $3}' | head -n 1)
...
while ! cardano-cli query utxo --address $address $testnet | grep -q $tokenname; do
  sleep 5
done
